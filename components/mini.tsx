...
export function MiniModel({ modelPath, cameraPosition, scale = 2 }: { modelPath?: string, cameraPosition?: [number, number, number], scale?: number }) {
  const [degraded, degrade] = useState(false);
  const { theme } = useTheme();

  return (
    <Canvas shadows camera={{ position: cameraPosition || [5, 0, 15], fov: 30 }}>
      <spotLight
        position={[0, 15, 0]}
        angle={0.3}
        penumbra={1}
...
function Mini(props: any) {
  const modelPath = props.modelPath || "/mini.glb";
  console.log('Mini component loading modelPath:', modelPath);
